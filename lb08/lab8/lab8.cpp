...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hbrBackground = CreateSolidBrush(RGB(255, 249, 200));
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	...
	listBox1 = CreateWindow(L"LISTBOX", L"Процессы", WS_CHILD | WS_VISIBLE | WS_BORDER | WS_VSCROLL | LBS_NOTIFY,
		20, 50, 250, 400, hWnd, (HMENU)LISTBOX1_ID, hInstance, NULL);
	listBox2 = CreateWindow(L"LISTBOX", L"Модули", WS_CHILD | WS_VISIBLE | WS_BORDER | WS_VSCROLL,
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CTLCOLORLISTBOX:
	{
		...
		SetBkColor(hdcStatic, RGB(248, 255, 245));
		return (INT_PTR)hBrush;
	}
	case WM_PAINT:
		hdc = BeginPaint(hWnd, &ps);
		SetTextColor(hdc, RGB(222, 30, 200));
		SetBkColor(hdc, RGB(255, 249, 200));
